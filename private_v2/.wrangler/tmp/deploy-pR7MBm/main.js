...
// main.js
var SERVER_DOMAIN = "app.scribbl.it";
var PAGES_DOMAIN = "scribbl.it";
var MODELS = {
  GEMINI_MODELS: {
...
__name(createPromptWithFileDescription, "createPromptWithFileDescription");
...
${textContent}`
    ...
    if (reasoning) {
      body.generation_config = {
        thinking_config: {
          thinking_budget: -1
          // let the model decide how long to think for
        }
      };
    } else {
      body.generation_config = {
...
__name(callCerebrasModel, "callCerebrasModel");
...
__name(callAnthropicModel, "callAnthropicModel");
  ...
  if (modelName === "qwen/qwen3-32b") {
    ...
    try {
      const groqRequest = {
        ...
        // we don't want to see the model thinking
      };
    ...
    switch (url.pathname) {
      case "/signup": {
        ...
        try {
          ...
          const verification_code_expires_at = Date.now() + 10 * 60 * 1e3;
          let user_id;
          if (existingUser) {
      ...
      case "/auth/google": {
        ...
        const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id=${env.GOOGLE_CLIENT_ID}&redirect_uri=${encodeURIComponent("https://" + SERVER_DOMAIN + "/auth/google/callback")}&scope=${encodeURIComponent("openid email")}&state=${state}`;
      ...
      case "/auth/google/callback": {
        ...
        try {
          ...
          let user = await env.DB.prepare("SELECT user_id, email, verified_email, data FROM users WHERE email = ?").bind(googleUser.email).first();
          let user_id;
          if (user) {
